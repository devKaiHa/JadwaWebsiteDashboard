...
const UpdateStagePage = () => {
  ...
  const getPlaceholder = (field, lang) => {
    ...
    if (field === "description") return `Enter description in ${lang.toUpperCase()}`;
    return "";
  };
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="card p-5 shadow-md rounded-xl">
          ...
          <div className="input-group mb-3">
            ...
            <input
              ...
              className={`input w-full ${errors[`title${lang.toUpperCase()}`] ? "input-error" : ""}`}
              disabled={isLoading}
            />
          </div>
        </div>
        ...
        <div className="card p-5 shadow-md rounded-xl">
          <div className="flex justify-between items-center mb-4">
            ...
            <button className="btn btn-primary btn-sm" onClick={addStep} disabled={isLoading}>
              Add Step
            </button>
          </div>
                  ...
                  <div className="input-group mb-2">
                    ...
                    {field === "description" ? (
                      <textarea
                        ...
                        onChange={(e) => handleStepChange(index, field, lang, e.target.value)}
                        className={`textarea w-full ${errors[`step${index + 1}_${field}${lang.toUpperCase()}`] ? "textarea-error" : ""}`}
                        rows={4}
                        disabled={isLoading}
                      />
                    ) : (
                      <input
                        ...
                        value={step[field]?.[lang] || ""}
                        onChange={(e) => handleStepChange(index, field, lang, e.target.value)}
                        className={`input w-full ${errors[`step${index + 1}_${field}${lang.toUpperCase()}`] ? "input-error" : ""}`}
                        disabled={isLoading}
                      />
                    )}
                  </div>
  ...
  return (
    <Container>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Update Stage</h1>
        <AddButton label="Save Stage" onClick={handleSave} disabled={isLoading} />
      </div>
