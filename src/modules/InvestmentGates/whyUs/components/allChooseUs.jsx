...
import useAllChooseUsHook from "../hooks/UseAllChooseUs";
...
const ChooseUsList = () => {
  ...
  const handleDelete = async () => {
    const success = await handleDeleteChooseUs();
    ...
    <Container>
      <div className="card min-w-full">
        ...
        <div className="card-header flex justify-between items-center">
          <h3 className="card-title">Choose Us List</h3>
          <div className="flex gap-2">
            <AddButton
              label="Add Choose Us"
              onClick={() => navigate("/investment-add-chooseUs")}
            />
            <AddButton
              label="Update Titles"
            ...
            <tbody>
              {allData?.map((item, index) => (
                <tr key={item._id} className="hover:bg-gray-50">
                  ...
                  <td className="max-w-xs truncate">{item.title?.ar || "-"}</td>
                  <td className="max-w-xs truncate">{item.title?.tr || "-"}</td>
                    ...
                    <div className="flex gap-4 cursor-pointer">
                      <Tooltip title="Edit" placement="top">
                        <button
                          ...
                          onClick={() =>
                            navigate(`/investment-update-chooseUs/${item._id}`)
                          }
                        >
